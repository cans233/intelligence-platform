...
import { authApi, getApiMode, setAccessToken } from '../api/client'
...
export function LoginPage({ onSuccess }: { onSuccess: () => void }) {
  ...
  return <div className="login-page">
    <div className="login-rail">
      ...
      <div className="rail-foot"><span className="status-dot" />{getApiMode() === 'mock' ? 'Contract Mock' : 'Real Core + Contract Mock'} · B Phase 2</div>
    </div>
    <div className="login-panel"><div className="login-inner">
      ...
      <Form form={form} layout="vertical" onFinish={submit} className="login-form" initialValues={{ username: 'admin', password: getApiMode() === 'mock' ? '1234' : 'Admin-Phase2-2026!' }}>
      ...
      <div className="login-note"><CloudServerOutlined /> {getApiMode() === 'mock' ? '当前为 Mock 数据环境，所有数据仅用于界面演示' : '核心事实使用真实 API，搜索与运营页保留 Contract Mock'}</div>
    </div></div>
  </div>
}
